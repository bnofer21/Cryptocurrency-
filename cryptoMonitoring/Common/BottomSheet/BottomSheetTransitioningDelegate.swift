...
final class BottomSheetPresentationController: UIPresentationController {
    ...
    override func presentationTransitionWillBegin() {
        ...
        if sheetSizingFactor > 1 {
            preferredHeightConstraint = presentedView.heightAnchor.constraint(
                equalToConstant: sheetSizingFactor
            )
        } else {
            preferredHeightConstraint = presentedView.heightAnchor.constraint(
...
extension BottomSheetPresentationController: UIGestureRecognizerDelegate {
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        touch.location(in: presentedView).y < 100
    }
}
